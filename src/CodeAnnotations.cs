...
namespace JetBrains.Annotations
{
    ...
    /// <summary>
    /// Can be appplied to symbols of types derived from IEnumerable as well as to symbols of Task and Lazy classes to indicate
    /// that the value of a collection item, of the Task.Result property or of the Lazy.Value property can never be null.
    /// </summary>
    [AttributeUsage(
        AttributeTargets.Method | AttributeTargets.Parameter | AttributeTargets.Property | AttributeTargets.Delegate |
    ...
    /// <summary>
    /// Can be appplied to symbols of types derived from IEnumerable as well as to symbols of Task and Lazy classes to indicate
    /// that the value of a collection item, of the Task.Result property or of the Lazy.Value property can be null.
    /// </summary>
    [AttributeUsage(
        AttributeTargets.Method | AttributeTargets.Parameter | AttributeTargets.Property | AttributeTargets.Delegate |
            AttributeTargets.Field)]
    ...
    /// <summary>
    /// Indicates that the marked method builds string by format pattern and (optional) arguments. Parameter, which contains
    /// format string, should be given in constructor. The format string should be in
    ...
    /// <summary>
    /// For a parameter that is expected to be one of the limited set of values. Specify fields of which type should be used as
    ...
    /// canbenull
    /// </c>
    /// annotation is only applicable for output parameters.<br />
    /// You can use multiple
    /// <c>
    /// [ContractAnnotation]
    /// </c>
    /// for each FDT row, or use single attribute with rows separated by semicolon.<br />
    /// </syntax>
    /// <examples>
    /// <list>
    /// <item>
    ...
    /// <summary>
    /// When applied to a target attribute, specifies a requirement for any type marked with the target attribute to implement
    /// or inherit specific type or types.
    /// </summary>
    /// <example>
    /// <code>
    ...
    /// <summary>
    /// Indicates that the marked symbol is used implicitly (e.g. via reflection, in external library), so this symbol will not
    /// be marked as unused (as well as by other usage inspections).
    /// </summary>
    [AttributeUsage(AttributeTargets.All)]
    [Conditional("JETBRAINS_ANNOTATIONS")]
    ...
    /// <summary>
    /// Should be used on attributes and causes ReSharper to not mark symbols marked with such attributes as unused (as well as
    /// by other usage inspections)
    /// </summary>
    ...
    /// <summary>
    /// Tells code analysis engine if the parameter is completely handled when the invoked method is on stack. If the parameter
    /// is a delegate, indicates that delegate is executed while the method is executed. If the parameter is an enumerable,
    ...
    /// <summary>
    /// An extension method marked with this attribute is processed by ReSharper code completion as a 'Source Template'. When
    /// extension method is completed over some expression, it's source code is automatically expanded like a template at call
    /// site.
    /// </summary>
    /// <remarks>
    /// Template method body can contain valid source code and/or special comments starting with '$'. Text inside these
    /// comments is added as source code when the template is applied. Template parameters can be used either as additional
    /// method parameters or as identifiers wrapped in two '$' signs. Use the <see cref="MacroAttribute" /> attribute to
    /// specify macros for parameters.
    /// </remarks>
    /// <example>
    ...
    /// <see cref="SourceTemplateAttribute">
    /// source template
    /// </see>
    /// .
    /// </summary>
    /// <remarks>
    /// You can apply the attribute on the whole method or on any of its additional parameters. The macro expression is defined
    /// in the <see cref="MacroAttribute.Expression" /> property. When applied on a method, the target template parameter is
    /// defined in the <see cref="MacroAttribute.Target" /> property. To apply the macro silently for the parameter, set the
    ...
    /// <summary>
    /// ASP.NET MVC attribute. If applied to a parameter, indicates that the parameter is an MVC action. If applied to a
    /// method, the MVC action name is calculated implicitly from the context. Use this attribute for custom wrappers similar
    /// to
    /// <c>
    ...
    /// <summary>
    /// ASP.NET MVC attribute. If applied to a parameter, indicates that the parameter is an MVC controller. If applied to a
    /// method, the MVC controller name is calculated implicitly from the context. Use this attribute for custom wrappers
    /// similar to
    /// <c>
    ...
    /// <summary>
    /// ASP.NET MVC attribute. Indicates that a parameter is an MVC model type. Use this attribute for custom wrappers similar
    /// to
    /// <c>
    ...
    /// method, the MVC partial view name is calculated implicitly from the context. Use this attribute for custom wrappers
    /// similar to
    /// <c>
    ...
    /// <summary>
    /// ASP.NET MVC attribute. Indicates that a parameter is an MVC template. Use this attribute for custom wrappers similar to
    /// <c>
    ...
    /// <summary>
    /// ASP.NET MVC attribute. If applied to a parameter, indicates that the parameter is an MVC view. If applied to a method,
    /// the MVC view name is calculated implicitly from the context. Use this attribute for custom wrappers similar to
    /// <c>
    ...
    /// <summary>
    /// ASP.NET MVC attribute. When applied to a parameter of an attribute, indicates that this parameter is an MVC action
    /// name.
    /// </summary>
    /// <example>
    /// <code>
    ...
    /// To set the condition, mark one of the parameters with
    ...
    /// ItemsSource
    /// </c>
    /// property and should be treated as
    /// <c>
    /// ItemsControl
    /// </c>
    ...
    /// <summary>
    /// XAML attibute. Indicates the property of some
    /// <c>
    /// BindingBase
    /// </c>
    /// -derived type, that is used to bind some item of
    /// <c>
    /// ItemsControl
    /// </c>
    /// -derived type. This annotation will enable the
    /// <c>
    /// DataContext
    /// </c>
    ...
    /// ItemsControl
    /// </c>
    /// type or marked with the <see cref="XamlItemsControlAttribute" /> attribute.
    /// </remarks>
